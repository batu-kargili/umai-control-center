...
For a single-VM customer demo with Oracle, Redis, OpenLDAP, and the three UMAI services on one host, use [customer/all-in-one-vm/README.md](./customer/all-in-one-vm/README.md).
...
The default quick-start keeps `CC_SESSION_SECURE=false` so login works over `http://localhost:3000`. When you place the UI behind HTTPS, set `CC_SESSION_SECURE=true`.
...
| `CC_SESSION_SECURE` | No | Set `true` only when the UI is served over HTTPS. Default: `false` for localhost quick-start |
...
## Custom Browser Extension ID

The control-center image build supports `NEXT_PUBLIC_UMAI_EXTENSION_ID` as a Docker build argument. If your organization distributes a private extension ID, rebuild the image with your value:

```bash
docker build \
  --build-arg NEXT_PUBLIC_UMAI_EXTENSION_ID=your_extension_id \
  -t my-registry/umai-enterprise-control-center:custom \
  .
```

Then point the deployment compose file at your rebuilt image tag.
